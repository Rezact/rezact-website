import { PluginOption } from "vite-client";
...
function findDependencies(startingNode, excludeDeps = {}) {
  ...
  walk.ancestor(startingNode, {
    ...
    MemberExpression(node: any) {
      ...
      identifiers[node.property.name] =
  ...
  return {
    ...
    transform(_src, id) {
      ...
      if (importsUsedArr.length > 0)
        magicString.prepend(
          `import {render, ${importsUsedArr.join(",")}} from "/rezact/index.js"\n`,
        );
      ...
      if (signalsUsedArr.length > 0)
        magicString.prepend(
          `import {${signalsUsedArr.join(",")}} from "/rezact/signals.js"\n`,
        );
      ...
      if (mapStateUsedArr.length > 0)
        magicString.prepend(
          `import {${mapStateUsedArr.join(",")}} from "/rezact/mapState.js"\n`,
        );
