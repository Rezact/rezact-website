import { createElement, setCreateCompFunc, isArray, addAppendChildHook, addAttributeHandler, addAfterRenderHook, createComment, handleInputValue, appendChild, createTextNode, createDocumentFragment } from './index.js';
...
resetBatchTimeout();
let subscFunctionsArr = [];
function createSignalElement(elm) {
  if (isArray(elm)) {
...
function _createComponent(tagName, attributes = null) {
  let previousSubscFuncs = subscFunctionsArr;
  subscFunctionsArr = tagName.subs = [];
  const compElm = tagName.func ? tagName.func(tagName.item, tagName.item.idxState) : tagName(attributes);
...
class Signal {
  ...
  set(newVal) {
    const val = this.get();
    if (newVal === val && !isArray(newVal))
      return;
    if (val instanceof Element && newVal instanceof Element && val !== newVal && val.parentNode)
      val.replaceWith(newVal);
    if (this.value instanceof Text) {
      this.value.nodeValue = newVal;
    } else {
      this.value = newVal;
    }
...
const computeSub = (obj) => obj.newState.set(obj.func(obj.deps));
function effect(func, deps) {
  const newState = new deps[0].constructor(func(deps));
...
function attrEffect(func, deps) {
  return { computer: func, deps };
}
const textTypes = { string: true, number: true };
function handleStateTypes(parent, child, ...args) {
  const val = child.get();
  if (textTypes[typeof val]) {
    ...
    child.subscribe((newVal) => {
      if (typeof newVal === "boolean" || newVal === null || newVal === void 0)
...
const childStateHandler = {
  matches: (child) => typeof child === "object" && (child.state || child.computer),
...
const attributeStateHandler = {
  matches: (_attrs, key, attrVal) => (
    ...
    !(handleInputValue && (key === "value" || key === "checked")) && (attrVal.state || attrVal.computer)
  ),
...
function handleTextNode(parent, child, ...args) {
  const val = child.get();
...
const afterSignalRenderHook = () => {
  subscFunctionsArr.forEach((func) => {
    func.stateObj.subs.set(func, true);
...
const addChildren = (values, parentNode) => {
  if (values === void 0)
    return;
...
const removeStaleChildren = (parentNode, endNode, parent, child) => {
  ...
  if (!parentNode.parentNode)
    return;
  if (parentNode.parentNode.childNodes.length === child.previousChildLen + 2 && values.length === 0) {
  ...
  while (nextNode !== endNode) {
    ...
    if (values.indexOf(nextNode.associatedState) < 0) {
      ...
      nextNode = nextNode.nextSibling;
      const prevElm = nextNode.previousSibling;
      if (prevElm.replaceWith && elmRef instanceof HTMLElement && elmRef.parentNode) {
        elmRef.replaceWith(remPlaceHolder);
        prevElm.replaceWith(elmRef);
        remPlaceHolder.replaceWith(prevElm);
      } else {
        if (elmRef instanceof HTMLElement)
          elmRef.remove();
      }
    } else {
      nextNode = nextNode.nextSibling;
...
const handleArray = (parent, child) => {
...
function getVal(signal) {
  return signal.value;
}

export { Signal, afterSignalRenderHook, attrEffect, computeSub, effect, getVal };
