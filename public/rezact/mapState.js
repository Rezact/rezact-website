import { Signal, afterSignalRenderHook, effect } from "./signals.js";
import { inRender, createComponent, isArray } from "./index.js";

function isIgnoredInstance(obj) {
  return (
  ...
  updateList(func) {
    ...
    for (let idx = 0; idx < len; idx++) {
      ...
      if (
        ...
        item.elmRef?.isConnected === void 0
      )
        item.elmRef = void 0;
...
const mapEffect = (func, deps) => {
  const computed = effect(func, deps);
  computed.deps = deps;
  computed.mapStateObj = false;
  return computed;
};

export { MapSignal, mapEffect };
